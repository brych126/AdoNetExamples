...
namespace AdoNetExamples
{
    ...
    {
        // In current example ID is not properly handled due to limitation of autogenerated commands
        // See examples below how to retrieve correctly ID
...
SET @Id = CAST(SCOPE_IDENTITY() AS int);", conn);
        ...
        public static void InsertCustomersInBatch_WithOutputParameters()
        {
            using var conn = new SqlConnection(AdoNetExamplesConnectionStringBuilder.ConnectionString);
            conn.Open();

            // 1) Adapter selects schema (we'll fill an empty table and add rows)
            using var adapter = new SqlDataAdapter(
                "SELECT Id, [Name], Email, CreatedAt FROM dbo.Customers WHERE 1 = 0;", conn);
            ...
            // 2) Build INSERT with OUTPUT parameter for identity
            // Important: no resultset OUTPUT INSERTED... because batching won't allow FirstReturnedRecord.
...
", conn);
            ...
            insert.Parameters.Add("@Name", SqlDbType.NVarChar, 100, "Name");
            SqlParameter? pEmail = insert.Parameters.Add("@Email", SqlDbType.NVarChar, 255, "Email");
            pEmail.IsNullable = true;

            // Output parameter mapped to DataTable column "Id"
            SqlParameter? pId = insert.Parameters.Add("@Id", SqlDbType.Int, 0, "Id");
            pId.Direction = ParameterDirection.Output;

            // REQUIRED for batching when you want values back:
            insert.UpdatedRowSource = UpdateRowSource.OutputParameters;

            adapter.InsertCommand = insert;

            // 3) Enable batching
            adapter.UpdateBatchSize = 1; // anything > 1 enables batching (20 is just an example)

            // Hook events
            int updatingCount = 0;
            int updatedCount = 0;

            adapter.RowUpdating += (_, e) =>
            {
                updatingCount++;
                // One per row always
                Console.WriteLine(
                    $"  RowUpdating  #{updatingCount}: StatementType={e.StatementType}, RowState={e.Row.RowState}");
            };

            adapter.RowUpdated += (_, e) =>
            {
                updatedCount++;
                // When batching enabled, this is per batch (not per row)
                Console.WriteLine($"  RowUpdated   #{updatedCount}: StatementType={e.StatementType}, Status={e.Status}," +
                                  $" RecordsAffected={e.RecordsAffected}, " +
                                  $"Errors={(e.Errors != null ? e.Errors.Message : "none")}");
            };

            // 4) Add multiple new rows
            table.Rows.Add(null, "Batch A", "a@example.com", DateTime.MinValue);
            table.Rows.Add(null, "Batch B", "b@example.com", DateTime.MinValue);
            table.Rows.Add(null, "Batch C", DBNull.Value, DateTime.MinValue);

            // Show Id before Update (will be DBNull)
            Console.WriteLine("Before Update():");
            foreach (DataRow r in table.Rows)
                Console.WriteLine($"  Name={r["Name"]}, Id={(r.IsNull("Id") ? "NULL" : r["Id"])}");

            Console.WriteLine(Environment.NewLine);

            // 5) Push changes in batch; output parameters populate Id per row
            adapter.Update(table);

            Console.WriteLine("\nAfter Update(): (Ids filled from OUTPUT param)");
            foreach (DataRow r in table.Rows)
                Console.WriteLine($"  Name={r["Name"]}, Id={r["Id"]}");

            // Note: after Update(), rows are usually Unchanged and Original == Current
            // because AcceptChangesDuringUpdate defaults to true.
        }
        ...
        public static void MySchoolDbAdapterUpdate()
        {
            ...
            void UpdateDepartmentName(DataRow rowToUpdate, string newName)
            {
                ...
                dataAdapter.Update(departmentTable);
            ...
            DataTableMapping[] tableMappings)
        {
            ...
            // If AcceptChangesDuringFill is false then loaded rows will be treated as newly added (RowState - Added)
            // Otherwise (RowState - Unchanged)
            ...
            DataColumn[] primaryColumns, DataColumn[] resetColumns)
        {
            ...
            string resetCols =
                string.Join(",", resetColumns.Select(col => $"Max({col.ColumnName}) as {col.ColumnName}"));
        ...
        private static void BatchInsertUpdate(DataTable table, string connectionString,
            SqlCommand insertCommand, int batchSize)
        {
            using var connection = new SqlConnection(connectionString);
